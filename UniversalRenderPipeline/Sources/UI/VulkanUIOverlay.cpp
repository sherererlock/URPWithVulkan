...
namespace vks 
{
	...
	void UIOverlay::prepareResources()
	{
...
#else
		const std::string filename = ENGINE_DIR + std::string("Roboto-Medium.ttf");
		...
		std::unique_ptr<ShBuffer> stagingBuffer = std::make_unique<ShBuffer>(*device,
			uploadSize, 1, 
	...
	bool UIOverlay::update()
	{
		...
		for (int n = 0; n < imDrawData->CmdListsCount; n++) {
			...
			vertexBuffer->writeToBuffer(cmd_list->VtxBuffer.Data, cmd_list->VtxBuffer.Size * sizeof(ImDrawVert));

			//memcpy(idxDst, cmd_list->IdxBuffer.Data, cmd_list->IdxBuffer.Size * sizeof(ImDrawIdx));
			indexBuffer->writeToBuffer(cmd_list->IdxBuffer.Data, cmd_list->IdxBuffer.Size * sizeof(ImDrawIdx));
